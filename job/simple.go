...
func simple(e *bandit.Experiment, s []bandit.Stats, logFile, snapshotFile string, poll time.Duration) error {
	...
	go func() {
		...
		for _ = range t.C {
			...
			mapper := bandit.SnapshotMapper(e, s, r, w)
			mapper()
			mapped := w.String()

			rS, w := strings.NewReader(mapped), new(bytes.Buffer)
			reducer := bandit.SnapshotReducer(e, s, rS, w)
			reducer()
